...
# https://www.kaggle.com/sudalairajkumar/santander-product-recommendation/when-less-is-more/code
...
def get_days_in(row, ref_time_str):
    '''
    Pocet dni od zalozenia ktorehokolvek produktu prvy krat.
    '''
    if row["fecha_alta"].strip() in ['NA', '']:
        datum = datetime.datetime(2012,7,1)
    else:
        datum = datetime.datetime.strptime(row["fecha_alta"], "%Y-%m-%d")
    ref_time = datetime.datetime.strptime(ref_time_str, "%Y-%m-%d")
    days_in = (ref_time - datum).days
    return days_in
...
def fill_series(*args):
    '''
    Ak zakaznik mal poslednych 5 mesiacov prerusenie sluzby, tak namiesto
    nulovych produktov v ten mesiac, sa skopiruju produkty z mesiaca pred.
    Vyrazne pohorsenie. Neuspech!!!

    prev_targets vstupuju od najstarsieho po najmladsi
    '''
    sums = []
    for prev_target in args:
        sums.append(sum(prev_target))

    fails = []
    check = False
    for i, suma in enumerate(sums):
        if suma == 0 and check:
            fails.append(i)
...
def sum_of_products(*args):
    '''
    sum of products for every month appended to list
    '''
    sums = []
    for prev_list in args:
        suma = sum(prev_list)
        sums.append(suma)
    return sums
...
def processData(in_file_name, cust_dict, cust_dict_04, cust_dict_03, cust_dict_02, cust_dict_01):
    ...
    for row in csv.DictReader(in_file_name):
        ...
        if row['fecha_dato'] == '2016-06-28':
            ...
            # produc_sums = sum_of_products(prev_target_list, prev_target_list_04, prev_target_list_03, prev_target_list_02, prev_target_list_01)
